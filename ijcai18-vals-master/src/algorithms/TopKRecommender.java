...
public abstract class TopKRecommender {
	...
	/**
	 * Print the hit user to the path
	 * @throws IOException 
	 * 
	 */
	public void showProgressWithHitUsers(int iter, long start, ArrayList<Rating> testRatings, BufferedWriter path) throws IOException {
		...
		path.write("Iter="+iter+"\n");
		for (int n = 0; n < userCount; n++) {
			if (hits.get(n)>0.5) 
				path.write(n+"\t"+testRatings.get(n).itemId+"\n");
		}
	}
